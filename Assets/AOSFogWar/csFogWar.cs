...
namespace FischlWorks_FogWar
{
    ...
    public class csFogWar : MonoBehaviour
    {
        ...
        public void RemoveFogRevealer(Transform targetTransform)
        {
            // LINQ를 사용해 targetTransform을 가진 FogRevealer를 찾습니다.
            var revealerToRemove = fogRevealers.FirstOrDefault(f => f._RevealerTransform == targetTransform);

            if (revealerToRemove != null)
            {
                fogRevealers.Remove(revealerToRemove);
            }
            else
            {
                Debug.Log("No revealer found with the specified Transform.");
            }
        }
