    ...
    public class RayfireRigidRoot : MonoBehaviour
    {
            ...
            for (int i = 0; i < children.Length; i++)
            {
                ...
                else
                {
                    ...
                    else if (rigid.objectType == ObjectType.ConnectedCluster)
                    {
                        ...
                        for (int r = 0; r < rigid.clusterDemolition.cluster.shards.Count; r++)
                            rigid.clusterDemolition.cluster.shards[r].rigid = rigid;

                        // Collect to all shards TODO create new shards
                        cluster.shards.AddRange (rigid.clusterDemolition.cluster.shards);
                    }
                }
            }
        ...
        void SetCustomShardsLists()
        {
            ...
            for (int i = 0; i < cluster.shards.Count; i++)
                ...
                else
                {
                     ...
                     else if (cluster.shards[i].rigid.objectType == ObjectType.ConnectedCluster) 
        ...
        void SetShardsSimulationType()
        {
            ...
            for (int i = 0; i < connClusterShards.Count; i++)
                connClusterShards[i].sm = connClusterShards[i].rigid.simulationType;
        }
