    ...
    public class RFActivation
    {
        ...
        public static bool ActivateShard (RFShard shard, RayfireRigidRoot rigidRoot)
        {
            ...
            if (shard.rigid != null)
            {
                ...
                if (shard.rigid.objectType == ObjectType.ConnectedCluster)
                {
                    ActivateRigid (shard.rigid);
                    return true;
                }
        ...
        {
            if (scr.objectType == ObjectType.ConnectedCluster && scr.rigidRoot != null)
            {
                for (int i = 0; i < scr.clusterDemolition.cluster.shards.Count; i++)
                    scr.clusterDemolition.cluster.shards[i].sm = SimType.Dynamic;
