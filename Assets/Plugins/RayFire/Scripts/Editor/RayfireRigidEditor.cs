    ...
    public class RayfireRigidEditor : Editor
    {
        ...
        void UI_Mesh_Props()
        {
            ...
            if (exp_prp == true)
            {
                ...
                EditorGUI.BeginChangeCheck();
                rigid.meshDemolition.prp.dec = EditorGUILayout.Toggle (gui_msh_adv_dec, rigid.meshDemolition.prp.dec);
                if (EditorGUI.EndChangeCheck())
                    foreach (RayfireRigid scr in targets)
                    {
                        scr.meshDemolition.prp.dec = rigid.meshDemolition.prp.dec;
                        SetDirty (scr);
                    }
                ...
                EditorGUI.BeginChangeCheck();
                rigid.meshDemolition.prp.cap = EditorGUILayout.Toggle (gui_msh_adv_cap, rigid.meshDemolition.prp.cap);
                if (EditorGUI.EndChangeCheck())
                    foreach (RayfireRigid scr in targets)
                    {
                        scr.meshDemolition.prp.cap = rigid.meshDemolition.prp.cap;
                        SetDirty (scr);
                    }
